...
static void render(struct display_output *output) {
    ...
    else{ //inform libmpv that he bufferahs been presented so it can release any
        // assciated GL fence objects and resources
        if (mpv_glcontext)
...
static void *get_proc_address_mpv(void *ctx, const char *name) {
    ...
    if (strcmp(name, "glFenceSync")== 0 ) {
        return (void*)wrap_glFenceSync;
    }
    if(strcmp(name, "glDeleteSync") == 0){
        return(void *) wrap_glDeleteSync;
    }
